# Q74：Business Intelligence (商业智能)（95分面试版）

## 1. 三句话题目本质
1. Business Intelligence (商业智能) 的核心是：围绕“BI报表、指标看板、钻取分析”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
数仓题核心是口径统一和可追溯，难点在维度慢变、指标版本和回填修复。
...
| BI报表 | BI报表是该题关键技术点 | 面试要说明BI报表的边界和取舍 |
| 指标看板 | 指标看板是该题关键技术点 | 面试要说明指标看板的边界和取舍 |
| 钻取分析 | 钻取分析是该题关键技术点 | 面试要说明钻取分析的边界和取舍 |
| 自助分析 | 自助分析是该题关键技术点 | 面试要说明自助分析的边界和取舍 |
| 权限 | 权限是该题关键技术点 | 面试要说明权限的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Business Intelligence 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 7. API 设计（请求/响应示例）
- `POST /metrics/query`
- `POST /metrics/definition`
- `POST /metrics/approve`
- `POST /backfill/jobs`

## 8. 数据模型（核心表/索引）
- `dim_xxx_scd2(sk,biz_id,valid_from,valid_to)`
- `fact_xxx(order_id,dt,amount)`
- `metric_def(code,version,sql_logic)`
...
### 15.1 幂等入口与主事务
```java
public class BusinessIntelligenceCommandService {
...
### 15.2 状态机与边界校验
```java
public class BusinessIntelligenceStateMachine {
...
### 15.3 失败重试与死信
```java
public class BusinessIntelligenceRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class BusinessIntelligenceQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 74 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class BusinessIntelligenceReconcileJob {
...
export function BusinessIntelligenceHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/74/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Business Intelligence (商业智能) - 指标目录页</h3>
      {msg && <p>{msg}</p>}
...
export function BusinessIntelligenceOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/74/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/74/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Business Intelligence (商业智能) - 口径详情与查询看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Business Intelligence (商业智能) 如何保证 指标定义与口径 不退化。
2. 当依赖超时时，Business Intelligence (商业智能) 如何通过降级保证主功能可用。
3. 当 看板搭建 出现冲突时，如何通过幂等和补偿收敛。
4. 当 多维钻取 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 权限隔离 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“指标定义与口径、看板搭建”展开。  
3. 失败模式重心不同：本题更常见“多维钻取”相关故障。  
4. 性能热点位置不同：本题重点在“BI报表”链路。  
5. 运营治理不同：本题要重点覆盖“指标看板”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. 指标定义与口径 的实现边界与替代方案  
2. 看板搭建 的一致性与容量取舍  
3. 多维钻取 的故障演练与止血策略  
4. 权限隔离 的指标阈值与诊断路径  
5. 性能与缓存 的落地代码与运维闭环  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 指标定义与口径，这是最显著差异。”  
2. “本题在 BI报表 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 指标看板 的落地与故障处置。”  
