# Q103：Design Message Queue System（95分面试版）

## 1. 三句话题目本质
1. Design Message Queue System 的核心是：围绕“消息队列、Kafka、分区”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
MQ系统核心是吞吐、顺序、重试和幂等，难点在积压与再平衡。
...
| 消息队列 | 消息队列是该题关键技术点 | 面试要说明消息队列的边界和取舍 |
| Kafka | Kafka是该题关键技术点 | 面试要说明Kafka的边界和取舍 |
| 分区 | 分区是该题关键技术点 | 面试要说明分区的边界和取舍 |
| 消费者组 | 消费者组是该题关键技术点 | 面试要说明消费者组的边界和取舍 |
| 持久化 | 持久化是该题关键技术点 | 面试要说明持久化的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Message Queue System 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
- `POST /consumer/commit`
- `GET /consumer/lag`

## 8. 数据模型（核心表/索引）
- `topic(topic,partitions,retention)`
- `offset_group(group_id,topic,partition,offset)`
- `dlq(topic,msg_id,reason)`
...
### 15.1 幂等入口与主事务
```java
public class MessageQueueSystemCommandService {
...
### 15.2 状态机与边界校验
```java
public class MessageQueueSystemStateMachine {
...
### 15.3 失败重试与死信
```java
public class MessageQueueSystemRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class MessageQueueSystemQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 103 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class MessageQueueSystemReconcileJob {
...
export function MessageQueueSystemHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/103/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Message Queue System - Topic管理页</h3>
      {msg && <p>{msg}</p>}
...
export function MessageQueueSystemOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/103/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/103/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Message Queue System - 消费组滞后面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Message Queue System 如何保证 Topic分区 不退化。
2. 当依赖超时时，Design Message Queue System 如何通过降级保证主功能可用。
3. 当 消费者组 出现冲突时，如何通过幂等和补偿收敛。
4. 当 Offset管理 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 消息持久化 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q9 Design Message Queue - 消息队列
...
2. 本题核心实体与索引设计围绕“Topic分区、消费者组”展开。  
3. 失败模式重心不同：本题更常见“Offset管理”相关故障。  
4. 性能热点位置不同：本题重点在“消息队列”链路。  
5. 运营治理不同：本题要重点覆盖“Kafka”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. Topic分区 的实现边界与替代方案  
2. 消费者组 的一致性与容量取舍  
3. Offset管理 的故障演练与止血策略  
4. 消息持久化 的指标阈值与诊断路径  
5. 高可用设计 的落地代码与运维闭环  

### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 Topic分区，这是最显著差异。”  
2. “本题在 消息队列 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 Kafka 的落地与故障处置。”  
