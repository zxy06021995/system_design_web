# Q95：Design Secure Identity Management System（95分面试版）

## 1. 三句话题目本质
1. Design Secure Identity Management System 的核心是：围绕“身份管理、SSO、OAuth”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
安全风控题核心是低延迟决策与误杀控制，难点在规则模型冲突和人审闭环。
...
| 身份管理 | 身份管理是该题关键技术点 | 面试要说明身份管理的边界和取舍 |
| SSO | SSO是该题关键技术点 | 面试要说明SSO的边界和取舍 |
| OAuth | OAuth是该题关键技术点 | 面试要说明OAuth的边界和取舍 |
| RBAC | RBAC是该题关键技术点 | 面试要说明RBAC的边界和取舍 |
| 认证 | 认证是该题关键技术点 | 面试要说明认证的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Identity Management System 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 7. API 设计（请求/响应示例）
- `POST /risk/evaluate`
- `POST /risk/rules/publish`
- `GET /risk/cases`
- `POST /risk/cases/{id}/review`

## 8. 数据模型（核心表/索引）
- `risk_event(event_id,score,decision)`
- `risk_rule(rule_id,expr,action)`
- `risk_case(case_id,status,result)`
...
### 15.1 幂等入口与主事务
```java
public class IdentityManagementSystemCommandService {
...
### 15.2 状态机与边界校验
```java
public class IdentityManagementSystemStateMachine {
...
### 15.3 失败重试与死信
```java
public class IdentityManagementSystemRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class IdentityManagementSystemQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 95 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class IdentityManagementSystemReconcileJob {
...
export function IdentityManagementSystemHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/95/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Secure Identity Management System - 风险审核台</h3>
      {msg && <p>{msg}</p>}
...
export function IdentityManagementSystemOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/95/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/95/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Secure Identity Management System - 规则发布面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Secure Identity Management System 如何保证 SSO设计 不退化。
2. 当依赖超时时，Design Secure Identity Management System 如何通过降级保证主功能可用。
3. 当 OAuth2.0 出现冲突时，如何通过幂等和补偿收敛。
4. 当 RBAC权限 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 多因素认证 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q93 Design Fraud Detection System
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“用户系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“SSO设计、OAuth2.0”展开。  
3. 失败模式重心不同：本题更常见“RBAC权限”相关故障。  
4. 性能热点位置不同：本题重点在“身份管理”链路。  
5. 运营治理不同：本题要重点覆盖“SSO”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. SSO设计 的实现边界与替代方案  
2. OAuth2.0 的一致性与容量取舍  
3. RBAC权限 的故障演练与止血策略  
4. 多因素认证 的指标阈值与诊断路径  
5. 审计日志 的落地代码与运维闭环  

### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 SSO设计，这是最显著差异。”  
2. “本题在 身份管理 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 SSO 的落地与故障处置。”  
