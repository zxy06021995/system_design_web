# Q79：Design Online Collaborative Spreadsheet（95分面试版）

## 1. 三句话题目本质
1. Design Online Collaborative Spreadsheet 的核心是：围绕“协同编辑、OT/CRDT、电子表格”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
协同编辑要在多人并发修改下保持文档收敛，核心是OT/CRDT和版本回放。
...
| 协同编辑 | 协同编辑是该题关键技术点 | 面试要说明协同编辑的边界和取舍 |
| OT/CRDT | OT/CRDT是该题关键技术点 | 面试要说明OT/CRDT的边界和取舍 |
| 电子表格 | 电子表格是该题关键技术点 | 面试要说明电子表格的边界和取舍 |
| 公式计算 | 公式计算是该题关键技术点 | 面试要说明公式计算的边界和取舍 |
| 实时同步 | 实时同步是该题关键技术点 | 面试要说明实时同步的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Collaborative Spreadsheet 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 8. 数据模型（核心表/索引）
- `doc(doc_id,head_version)`
- `doc_op(op_id,doc_id,base_version)`
- `doc_snapshot(doc_id,version,content_blob)`
...
### 15.1 幂等入口与主事务
```java
public class CollaborativeSpreadsheetCommandService {
...
### 15.2 状态机与边界校验
```java
public class CollaborativeSpreadsheetStateMachine {
...
### 15.3 失败重试与死信
```java
public class CollaborativeSpreadsheetRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class CollaborativeSpreadsheetQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 79 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class CollaborativeSpreadsheetReconcileJob {
...
export function CollaborativeSpreadsheetHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/79/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Online Collaborative Spreadsheet - 在线编辑页</h3>
      {msg && <p>{msg}</p>}
...
export function CollaborativeSpreadsheetOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/79/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/79/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Online Collaborative Spreadsheet - 版本对比页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Online Collaborative Spreadsheet 如何保证 OT/CRDT算法 不退化。
2. 当依赖超时时，Design Online Collaborative Spreadsheet 如何通过降级保证主功能可用。
3. 当 单元格锁定 出现冲突时，如何通过幂等和补偿收敛。
4. 当 公式计算 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 版本控制 上升时，值班如何按 Runbook 处置。
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“协同编辑系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“OT/CRDT算法、单元格锁定”展开。  
3. 失败模式重心不同：本题更常见“公式计算”相关故障。  
4. 性能热点位置不同：本题重点在“协同编辑”链路。  
5. 运营治理不同：本题要重点覆盖“OT/CRDT”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. OT/CRDT算法 的实现边界与替代方案  
2. 单元格锁定 的一致性与容量取舍  
3. 公式计算 的故障演练与止血策略  
4. 版本控制 的指标阈值与诊断路径  
5. 实时同步 的落地代码与运维闭环  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 OT/CRDT算法，这是最显著差异。”  
2. “本题在 协同编辑 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 OT/CRDT 的落地与故障处置。”  
