...
## 19. 面试追问 + 可复述回答
### Q1：为什么用 MQ，不直接调度器调用执行器？
可复述：解耦触发和执行，吸收流量突刺；调度器只保证 run 被可靠生成，执行速率由 worker 池和配额控制。  

### Q2：如何保证“同一时间片只触发一次”？
可复述：`(job_id, scheduled_time)` 唯一索引 + 幂等键 + 事务内写 run/outbox 三层保护。  

### Q3：高峰下最先做什么降级？
可复述：先保护核心租户和核心任务，暂停低优先级队列，再扩 worker；不是一上来盲目重试。  

### Q4：冷热分层具体收益是什么？
可复述：热库保最近 7 天高频查询，旧日志归档对象存储，查询稍慢但成本显著下降。  
