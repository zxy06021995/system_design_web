# Q101：Design Craigslist（95分面试版）

## 1. 三句话题目本质
1. Design Craigslist 的核心是：围绕“分类信息、搜索、地域”把业务主链路稳定跑通。  
...
| 分类信息 | 分类信息是该题关键技术点 | 面试要说明分类信息的边界和取舍 |
| 搜索 | 搜索是该题关键技术点 | 面试要说明搜索的边界和取舍 |
| 地域 | 地域是该题关键技术点 | 面试要说明地域的边界和取舍 |
| 发布 | 发布是该题关键技术点 | 面试要说明发布的边界和取舍 |
| 反垃圾 | 反垃圾是该题关键技术点 | 面试要说明反垃圾的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Craigslist 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class CraigslistCommandService {
...
### 15.2 状态机与边界校验
```java
public class CraigslistStateMachine {
...
### 15.3 失败重试与死信
```java
public class CraigslistRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class CraigslistQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 101 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class CraigslistReconcileJob {
...
export function CraigslistHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/101/health");
      if (resp.ok) {
...
export function CraigslistOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/101/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/101/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Craigslist - 审计追踪页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Craigslist 如何保证 分类管理 不退化。
2. 当依赖超时时，Design Craigslist 如何通过降级保证主功能可用。
3. 当 地域过滤 出现冲突时，如何通过幂等和补偿收敛。
4. 当 搜索优化 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 反垃圾 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“分类管理、地域过滤”展开。  
3. 失败模式重心不同：本题更常见“搜索优化”相关故障。  
4. 性能热点位置不同：本题重点在“分类信息”链路。  
5. 运营治理不同：本题要重点覆盖“搜索”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 分类管理，这是最显著差异。”  
2. “本题在 分类信息 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 搜索 的落地与故障处置。”  
