# Q77：Design High-Performance Computing Cluster（95分面试版）

## 1. 三句话题目本质
1. Design High-Performance Computing Cluster 的核心是：围绕“MPI、并行计算、作业调度”把业务主链路稳定跑通。  
...
| MPI | MPI是该题关键技术点 | 面试要说明MPI的边界和取舍 |
| 并行计算 | 并行计算是该题关键技术点 | 面试要说明并行计算的边界和取舍 |
| 作业调度 | 作业调度是该题关键技术点 | 面试要说明作业调度的边界和取舍 |
| 高性能 | 高性能是该题关键技术点 | 面试要说明高性能的边界和取舍 |
| 集群 | 集群是该题关键技术点 | 面试要说明集群的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 HPC Cluster 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class HPCClusterCommandService {
...
### 15.2 状态机与边界校验
```java
public class HPCClusterStateMachine {
...
### 15.3 失败重试与死信
```java
public class HPCClusterRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class HPCClusterQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 77 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class HPCClusterReconcileJob {
...
export function HPCClusterHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/77/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design High-Performance Computing Cluster - 发布回滚台</h3>
      {msg && <p>{msg}</p>}
...
export function HPCClusterOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/77/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/77/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design High-Performance Computing Cluster - 节点健康面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design High-Performance Computing Cluster 如何保证 MPI通信 不退化。
2. 当依赖超时时，Design High-Performance Computing Cluster 如何通过降级保证主功能可用。
3. 当 作业调度 出现冲突时，如何通过幂等和补偿收敛。
4. 当 并行计算 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 共享存储 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“MPI通信、作业调度”展开。  
3. 失败模式重心不同：本题更常见“并行计算”相关故障。  
4. 性能热点位置不同：本题重点在“MPI”链路。  
5. 运营治理不同：本题要重点覆盖“并行计算”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 MPI通信，这是最显著差异。”  
2. “本题在 MPI 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 并行计算 的落地与故障处置。”  
