...
### 22.3 差异
1. Q63 重点是企业本地协议到云对象存储的网关转换。
2. Q63 更强调站点断网容错与异步回传。
3. Q31 更偏用户端产品体验与多端协作。

### 22.4 本题新增必补知识（>=5）
1. WAL 落盘与断点恢复机制。
2. Sync Cursor 增量同步策略。
3. 站点级限速与优先级同步队列。
4. 对象存储请求成本模型与小文件合并。
5. 版本向量冲突检测与双版本保留。

### 22.5 面试话术
母题是网盘产品设计，本题是企业“本地存储接云”的中间层控制面，要突出协议转换、断网容错和同步治理。
