# Q100：Design Load Balancer（95分面试版）

## 1. 三句话题目本质
1. Design Load Balancer 的核心是：围绕“负载均衡、LVS、Nginx”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
网关在入口承受所有流量，必须在鉴权、限流、路由和熔断上先止血再放行。
...
| 负载均衡 | 负载均衡是该题关键技术点 | 面试要说明负载均衡的边界和取舍 |
| LVS | LVS是该题关键技术点 | 面试要说明LVS的边界和取舍 |
| Nginx | Nginx是该题关键技术点 | 面试要说明Nginx的边界和取舍 |
| 健康检查 | 健康检查是该题关键技术点 | 面试要说明健康检查的边界和取舍 |
| 算法 | 算法是该题关键技术点 | 面试要说明算法的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Load Balancer 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 7. API 设计（请求/响应示例）
- `POST /gateway/routes`
- `POST /gateway/ratelimit/rules`
- `GET /gateway/metrics`
- `POST /gateway/circuit/open`

## 8. 数据模型（核心表/索引）
- `route_rule(path,upstream,weight)`
- `rate_bucket(key,tokens,updated_at)`
- `gateway_log(trace_id,status,latency_ms)`
...
### 15.1 幂等入口与主事务
```java
public class LoadBalancerCommandService {
...
### 15.2 状态机与边界校验
```java
public class LoadBalancerStateMachine {
...
### 15.3 失败重试与死信
```java
public class LoadBalancerRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class LoadBalancerQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 100 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class LoadBalancerReconcileJob {
...
export function LoadBalancerHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/100/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Load Balancer - 路由管理台</h3>
      {msg && <p>{msg}</p>}
...
export function LoadBalancerOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/100/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/100/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Load Balancer - 限流规则面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Load Balancer 如何保证 轮询算法 不退化。
2. 当依赖超时时，Design Load Balancer 如何通过降级保证主功能可用。
3. 当 最少连接 出现冲突时，如何通过幂等和补偿收敛。
4. 当 IP哈希 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 健康检查 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q16 RateLimiter + Grafana - 限流监控
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“负载均衡”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“轮询算法、最少连接”展开。  
3. 失败模式重心不同：本题更常见“IP哈希”相关故障。  
4. 性能热点位置不同：本题重点在“负载均衡”链路。  
5. 运营治理不同：本题要重点覆盖“LVS”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 轮询算法，这是最显著差异。”  
2. “本题在 负载均衡 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 LVS 的落地与故障处置。”  
