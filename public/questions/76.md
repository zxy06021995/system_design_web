# Q76：Design Blockchain-Based System（95分面试版）

## 1. 三句话题目本质
1. Design Blockchain-Based System 的核心是：围绕“区块链、共识算法、智能合约”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
P2P题核心是节点发现、路由收敛和副本修复，难点在churn与NAT穿透。
...
| 区块链 | 区块链是该题关键技术点 | 面试要说明区块链的边界和取舍 |
| 共识算法 | 共识算法是该题关键技术点 | 面试要说明共识算法的边界和取舍 |
| 智能合约 | 智能合约是该题关键技术点 | 面试要说明智能合约的边界和取舍 |
| 加密 | 加密是该题关键技术点 | 面试要说明加密的边界和取舍 |
| 分布式账本 | 分布式账本是该题关键技术点 | 面试要说明分布式账本的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Blockchain System 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 8. 数据模型（核心表/索引）
- `peer_node(peer_id,status,last_seen)`
- `dht_record(key,value_ref,version)`
- `replica_mapping(key,peer_id,state)`
...
### 15.1 幂等入口与主事务
```java
public class BlockchainSystemCommandService {
...
### 15.2 状态机与边界校验
```java
public class BlockchainSystemStateMachine {
...
### 15.3 失败重试与死信
```java
public class BlockchainSystemRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class BlockchainSystemQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 76 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class BlockchainSystemReconcileJob {
...
export function BlockchainSystemHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/76/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Blockchain-Based System - 节点健康面板</h3>
      {msg && <p>{msg}</p>}
...
export function BlockchainSystemOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/76/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/76/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Blockchain-Based System - lookup路径追踪页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Blockchain-Based System 如何保证 PoW/PoS共识 不退化。
2. 当依赖超时时，Design Blockchain-Based System 如何通过降级保证主功能可用。
3. 当 默克尔树 出现冲突时，如何通过幂等和补偿收敛。
4. 当 智能合约 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 钱包安全 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q90 Design Peer-to-Peer Network
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“区块链系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“PoW/PoS共识、默克尔树”展开。  
3. 失败模式重心不同：本题更常见“智能合约”相关故障。  
4. 性能热点位置不同：本题重点在“区块链”链路。  
5. 运营治理不同：本题要重点覆盖“共识算法”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. PoW/PoS共识 的实现边界与替代方案  
2. 默克尔树 的一致性与容量取舍  
3. 智能合约 的故障演练与止血策略  
4. 钱包安全 的指标阈值与诊断路径  
5. 交易验证 的落地代码与运维闭环  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 PoW/PoS共识，这是最显著差异。”  
2. “本题在 区块链 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 共识算法 的落地与故障处置。”  
