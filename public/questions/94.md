# Q94：Design Secure Payment Gateway（95分面试版）

## 1. 三句话题目本质
1. Design Secure Payment Gateway 的核心是：围绕“支付网关、安全、加密”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
支付题第一目标是账务正确，难点在状态机、回调幂等、清结算和对账补偿。
...
| 支付网关 | 支付网关是该题关键技术点 | 面试要说明支付网关的边界和取舍 |
| 安全 | 安全是该题关键技术点 | 面试要说明安全的边界和取舍 |
| 加密 | 加密是该题关键技术点 | 面试要说明加密的边界和取舍 |
| PCI-DSS | PCI-DSS是该题关键技术点 | 面试要说明PCI-DSS的边界和取舍 |
| 风控 | 风控是该题关键技术点 | 面试要说明风控的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Secure Payment Gateway 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
- `POST /channels/callback`

## 8. 数据模型（核心表/索引）
- `payment_order(payment_id,status,amount)`
- `ledger_entry(entry_id,payment_id,direction)`
- `reconcile_result(payment_id,diff_type)`
...
### 15.1 幂等入口与主事务
```java
public class SecurePaymentGatewayCommandService {
...
### 15.2 状态机与边界校验
```java
public class SecurePaymentGatewayStateMachine {
...
### 15.3 失败重试与死信
```java
public class SecurePaymentGatewayRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class SecurePaymentGatewayQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 94 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class SecurePaymentGatewayReconcileJob {
...
export function SecurePaymentGatewayHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/94/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Secure Payment Gateway - 收银台页</h3>
      {msg && <p>{msg}</p>}
...
export function SecurePaymentGatewayOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/94/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/94/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Secure Payment Gateway - 支付状态页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Secure Payment Gateway 如何保证 数据加密 不退化。
2. 当依赖超时时，Design Secure Payment Gateway 如何通过降级保证主功能可用。
3. 当 PCI-DSS合规 出现冲突时，如何通过幂等和补偿收敛。
4. 当 风控规则 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 安全认证 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q84 Design Online Payment System (PayPal)
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“金融/支付系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“数据加密、PCI-DSS合规”展开。  
3. 失败模式重心不同：本题更常见“风控规则”相关故障。  
4. 性能热点位置不同：本题重点在“支付网关”链路。  
5. 运营治理不同：本题要重点覆盖“安全”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. 数据加密 的实现边界与替代方案  
2. PCI-DSS合规 的一致性与容量取舍  
3. 风控规则 的故障演练与止血策略  
4. 安全认证 的指标阈值与诊断路径  
5. 日志审计 的落地代码与运维闭环  

### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 数据加密，这是最显著差异。”  
2. “本题在 支付网关 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 安全 的落地与故障处置。”  
