# Q80：Design Ad Click Aggregation System（95分面试版）

## 1. 三句话题目本质
1. Design Ad Click Aggregation System 的核心是：围绕“点击统计、实时聚合、数据分析”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
推荐系统必须平衡效果与延迟，核心是召回-粗排-精排-重排闭环。
...
| 点击统计 | 点击统计是该题关键技术点 | 面试要说明点击统计的边界和取舍 |
| 实时聚合 | 实时聚合是该题关键技术点 | 面试要说明实时聚合的边界和取舍 |
| 数据分析 | 数据分析是该题关键技术点 | 面试要说明数据分析的边界和取舍 |
| 广告系统 | 广告系统是该题关键技术点 | 面试要说明广告系统的边界和取舍 |
| 流处理 | 流处理是该题关键技术点 | 面试要说明流处理的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Ad Click Aggregation 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 8. 数据模型（核心表/索引）
- `user_feature(user_id,vec)`
- `item_feature(item_id,vec)`
- `reco_event(event_id,user_id,item_id,type)`
...
### 15.1 幂等入口与主事务
```java
public class AdClickAggregationCommandService {
...
### 15.2 状态机与边界校验
```java
public class AdClickAggregationStateMachine {
...
### 15.3 失败重试与死信
```java
public class AdClickAggregationRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class AdClickAggregationQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 80 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class AdClickAggregationReconcileJob {
...
export function AdClickAggregationHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/80/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Ad Click Aggregation System - 推荐配置台</h3>
      {msg && <p>{msg}</p>}
...
export function AdClickAggregationOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/80/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/80/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Ad Click Aggregation System - AB实验看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Ad Click Aggregation System 如何保证 流式处理 不退化。
2. 当依赖超时时，Design Ad Click Aggregation System 如何通过降级保证主功能可用。
3. 当 实时聚合 出现冲突时，如何通过幂等和补偿收敛。
4. 当 数据存储 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 报表生成 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q36 Design Amazon Recommendation System
...
2. 本题核心实体与索引设计围绕“流式处理、实时聚合”展开。  
3. 失败模式重心不同：本题更常见“数据存储”相关故障。  
4. 性能热点位置不同：本题重点在“点击统计”链路。  
5. 运营治理不同：本题要重点覆盖“实时聚合”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 流式处理，这是最显著差异。”  
2. “本题在 点击统计 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 实时聚合 的落地与故障处置。”  
