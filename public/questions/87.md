# Q87：Design Ebook Distribution Platform（95分面试版）

## 1. 三句话题目本质
1. Design Ebook Distribution Platform 的核心是：围绕“电子书、DRM、格式转换”把业务主链路稳定跑通。  
...
| 电子书 | 电子书是该题关键技术点 | 面试要说明电子书的边界和取舍 |
| DRM | DRM是该题关键技术点 | 面试要说明DRM的边界和取舍 |
| 格式转换 | 格式转换是该题关键技术点 | 面试要说明格式转换的边界和取舍 |
| 阅读进度 | 阅读进度是该题关键技术点 | 面试要说明阅读进度的边界和取舍 |
| 推荐 | 推荐是该题关键技术点 | 面试要说明推荐的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Ebook Distribution Platform 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class EbookDistributionPlatformCommandService {
...
### 15.2 状态机与边界校验
```java
public class EbookDistributionPlatformStateMachine {
...
### 15.3 失败重试与死信
```java
public class EbookDistributionPlatformRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class EbookDistributionPlatformQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 87 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class EbookDistributionPlatformReconcileJob {
...
export function EbookDistributionPlatformHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/87/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Ebook Distribution Platform - 媒体上传台</h3>
      {msg && <p>{msg}</p>}
...
export function EbookDistributionPlatformOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/87/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/87/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Ebook Distribution Platform - 播放质量看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Ebook Distribution Platform 如何保证 DRM保护 不退化。
2. 当依赖超时时，Design Ebook Distribution Platform 如何通过降级保证主功能可用。
3. 当 格式转换 出现冲突时，如何通过幂等和补偿收敛。
4. 当 阅读进度同步 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 内容推荐 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“DRM保护、格式转换”展开。  
3. 失败模式重心不同：本题更常见“阅读进度同步”相关故障。  
4. 性能热点位置不同：本题重点在“电子书”链路。  
5. 运营治理不同：本题要重点覆盖“DRM”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 DRM保护，这是最显著差异。”  
2. “本题在 电子书 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 DRM 的落地与故障处置。”  
