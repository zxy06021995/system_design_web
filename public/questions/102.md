# Q102：Design Food Delivery Service（95分面试版）

## 1. 三句话题目本质
1. Design Food Delivery Service 的核心是：围绕“外卖、订单调度、配送”把业务主链路稳定跑通。  
...
| 外卖 | 外卖是该题关键技术点 | 面试要说明外卖的边界和取舍 |
| 订单调度 | 订单调度是该题关键技术点 | 面试要说明订单调度的边界和取舍 |
| 配送 | 配送是该题关键技术点 | 面试要说明配送的边界和取舍 |
| 实时定位 | 实时定位是该题关键技术点 | 面试要说明实时定位的边界和取舍 |
| 商家管理 | 商家管理是该题关键技术点 | 面试要说明商家管理的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Food Delivery Service 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class FoodDeliveryServiceCommandService {
...
### 15.2 状态机与边界校验
```java
public class FoodDeliveryServiceStateMachine {
...
### 15.3 失败重试与死信
```java
public class FoodDeliveryServiceRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class FoodDeliveryServiceQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 102 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class FoodDeliveryServiceReconcileJob {
...
export function FoodDeliveryServiceHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/102/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Food Delivery Service - 地图调度台</h3>
      {msg && <p>{msg}</p>}
...
export function FoodDeliveryServiceOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/102/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/102/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Food Delivery Service - 实时轨迹面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Food Delivery Service 如何保证 订单调度 不退化。
2. 当依赖超时时，Design Food Delivery Service 如何通过降级保证主功能可用。
3. 当 配送路径规划 出现冲突时，如何通过幂等和补偿收敛。
4. 当 实时定位 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 商家管理 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“订单调度、配送路径规划”展开。  
3. 失败模式重心不同：本题更常见“实时定位”相关故障。  
4. 性能热点位置不同：本题重点在“外卖”链路。  
5. 运营治理不同：本题要重点覆盖“订单调度”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 订单调度，这是最显著差异。”  
2. “本题在 外卖 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 订单调度 的落地与故障处置。”  
