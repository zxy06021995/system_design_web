# Q98：Design Podcast Hosting Platform（95分面试版）

## 1. 三句话题目本质
1. Design Podcast Hosting Platform 的核心是：围绕“播客、RSS、音频托管”把业务主链路稳定跑通。  
...
| 播客 | 播客是该题关键技术点 | 面试要说明播客的边界和取舍 |
| RSS | RSS是该题关键技术点 | 面试要说明RSS的边界和取舍 |
| 音频托管 | 音频托管是该题关键技术点 | 面试要说明音频托管的边界和取舍 |
| 订阅 | 订阅是该题关键技术点 | 面试要说明订阅的边界和取舍 |
| 发现 | 发现是该题关键技术点 | 面试要说明发现的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Podcast Hosting Platform 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class PodcastHostingPlatformCommandService {
...
### 15.2 状态机与边界校验
```java
public class PodcastHostingPlatformStateMachine {
...
### 15.3 失败重试与死信
```java
public class PodcastHostingPlatformRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class PodcastHostingPlatformQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 98 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class PodcastHostingPlatformReconcileJob {
...
export function PodcastHostingPlatformHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/98/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Podcast Hosting Platform - 媒体上传台</h3>
      {msg && <p>{msg}</p>}
...
export function PodcastHostingPlatformOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/98/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/98/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Podcast Hosting Platform - 播放质量看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Podcast Hosting Platform 如何保证 RSS生成 不退化。
2. 当依赖超时时，Design Podcast Hosting Platform 如何通过降级保证主功能可用。
3. 当 音频CDN 出现冲突时，如何通过幂等和补偿收敛。
4. 当 订阅管理 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 内容发现 上升时，值班如何按 Runbook 处置。
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“内容管理系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“RSS生成、音频CDN”展开。  
3. 失败模式重心不同：本题更常见“订阅管理”相关故障。  
4. 性能热点位置不同：本题重点在“播客”链路。  
5. 运营治理不同：本题要重点覆盖“RSS”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 RSS生成，这是最显著差异。”  
2. “本题在 播客 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 RSS 的落地与故障处置。”  
