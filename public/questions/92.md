# Q92：Design GPS Navigation System（95分面试版）

## 1. 三句话题目本质
1. Design GPS Navigation System 的核心是：围绕“GPS、路径规划、地图”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
地理调度题核心是实时位置更新和最优匹配，难点是定位噪声与高峰调度。
...
| GPS | GPS是该题关键技术点 | 面试要说明GPS的边界和取舍 |
| 路径规划 | 路径规划是该题关键技术点 | 面试要说明路径规划的边界和取舍 |
| 地图 | 地图是该题关键技术点 | 面试要说明地图的边界和取舍 |
| 导航 | 导航是该题关键技术点 | 面试要说明导航的边界和取舍 |
| 实时路况 | 实时路况是该题关键技术点 | 面试要说明实时路况的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 GPS Navigation System 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
## 8. 数据模型（核心表/索引）
- `geo_point(entity_id,lat,lng,ts)`
- `dispatch_order(order_id,status,driver_id)`
...
### 15.1 幂等入口与主事务
```java
public class GPSNavigationSystemCommandService {
...
### 15.2 状态机与边界校验
```java
public class GPSNavigationSystemStateMachine {
...
### 15.3 失败重试与死信
```java
public class GPSNavigationSystemRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class GPSNavigationSystemQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 92 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class GPSNavigationSystemReconcileJob {
...
export function GPSNavigationSystemHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/92/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design GPS Navigation System - 地图调度台</h3>
      {msg && <p>{msg}</p>}
...
export function GPSNavigationSystemOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/92/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/92/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design GPS Navigation System - 实时轨迹面板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design GPS Navigation System 如何保证 地图数据 不退化。
2. 当依赖超时时，Design GPS Navigation System 如何通过降级保证主功能可用。
3. 当 路径规划算法 出现冲突时，如何通过幂等和补偿收敛。
4. 当 实时路况 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 语音导航 上升时，值班如何按 Runbook 处置。
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“地理位置服务”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“地图数据、路径规划算法”展开。  
3. 失败模式重心不同：本题更常见“实时路况”相关故障。  
4. 性能热点位置不同：本题重点在“GPS”链路。  
5. 运营治理不同：本题要重点覆盖“路径规划”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 地图数据，这是最显著差异。”  
2. “本题在 GPS 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 路径规划 的落地与故障处置。”  
