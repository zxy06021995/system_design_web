# Q86：Design Music Streaming Service (Spotify)（95分面试版）

## 1. 三句话题目本质
1. Design Music Streaming Service (Spotify) 的核心是：围绕“音频流、推荐系统、播放列表”把业务主链路稳定跑通。  
...
| 音频流 | 音频流是该题关键技术点 | 面试要说明音频流的边界和取舍 |
| 推荐系统 | 推荐系统是该题关键技术点 | 面试要说明推荐系统的边界和取舍 |
| 播放列表 | 播放列表是该题关键技术点 | 面试要说明播放列表的边界和取舍 |
| DRM | DRM是该题关键技术点 | 面试要说明DRM的边界和取舍 |
| CDN | CDN是该题关键技术点 | 面试要说明CDN的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Music Streaming (Spotify) 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class MusicStreamingSpotifyCommandService {
...
### 15.2 状态机与边界校验
```java
public class MusicStreamingSpotifyStateMachine {
...
### 15.3 失败重试与死信
```java
public class MusicStreamingSpotifyRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class MusicStreamingSpotifyQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 86 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class MusicStreamingSpotifyReconcileJob {
...
export function MusicStreamingSpotifyHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/86/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Music Streaming Service (Spotify) - 媒体上传台</h3>
      {msg && <p>{msg}</p>}
...
export function MusicStreamingSpotifyOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/86/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/86/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Music Streaming Service (Spotify) - 播放质量看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Music Streaming Service (Spotify) 如何保证 音频流传输 不退化。
2. 当依赖超时时，Design Music Streaming Service (Spotify) 如何通过降级保证主功能可用。
3. 当 DRM保护 出现冲突时，如何通过幂等和补偿收敛。
4. 当 推荐算法 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 播放列表 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“音频流传输、DRM保护”展开。  
3. 失败模式重心不同：本题更常见“推荐算法”相关故障。  
4. 性能热点位置不同：本题重点在“音频流”链路。  
5. 运营治理不同：本题要重点覆盖“推荐系统”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 音频流传输，这是最显著差异。”  
2. “本题在 音频流 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 推荐系统 的落地与故障处置。”  
