# Q75：Design Smart Home System（95分面试版）

## 1. 三句话题目本质
1. Design Smart Home System 的核心是：围绕“IoT、设备管理、场景联动”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
通用平台题核心是平台抽象和权限治理，难点在灵活性与可控性的平衡。
...
| IoT | IoT是该题关键技术点 | 面试要说明IoT的边界和取舍 |
| 设备管理 | 设备管理是该题关键技术点 | 面试要说明设备管理的边界和取舍 |
| 场景联动 | 场景联动是该题关键技术点 | 面试要说明场景联动的边界和取舍 |
| 语音控制 | 语音控制是该题关键技术点 | 面试要说明语音控制的边界和取舍 |
| 自动化 | 自动化是该题关键技术点 | 面试要说明自动化的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Smart Home System 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
- `GET /platform/audit`
- `POST /platform/rollback`

## 8. 数据模型（核心表/索引）
- `app_meta(app_id,owner,status)`
- `config_item(key,val,ver)`
- `audit_log(actor,action,ts)`
...
### 15.1 幂等入口与主事务
```java
public class SmartHomeSystemCommandService {
...
### 15.2 状态机与边界校验
```java
public class SmartHomeSystemStateMachine {
...
### 15.3 失败重试与死信
```java
public class SmartHomeSystemRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class SmartHomeSystemQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 75 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class SmartHomeSystemReconcileJob {
...
export function SmartHomeSystemHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/75/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Smart Home System - 平台配置台</h3>
      {msg && <p>{msg}</p>}
...
export function SmartHomeSystemOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/75/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/75/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Smart Home System - 审计追踪页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Smart Home System 如何保证 MQTT协议 不退化。
2. 当依赖超时时，Design Smart Home System 如何通过降级保证主功能可用。
3. 当 设备发现 出现冲突时，如何通过幂等和补偿收敛。
4. 当 场景自动化 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 语音集成 上升时，值班如何按 Runbook 处置。
...
## 22. 与母题差异
### 22.1 对应母题
- 母题：Q60 Design Task Management Application
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“智能家居”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“MQTT协议、设备发现”展开。  
3. 失败模式重心不同：本题更常见“场景自动化”相关故障。  
4. 性能热点位置不同：本题重点在“IoT”链路。  
5. 运营治理不同：本题要重点覆盖“设备管理”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 MQTT协议，这是最显著差异。”  
2. “本题在 IoT 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 设备管理 的落地与故障处置。”  
