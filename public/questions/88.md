# Q88：Design Video Conferencing System（95分面试版）

## 1. 三句话题目本质
1. Design Video Conferencing System 的核心是：围绕“WebRTC、SFU、屏幕共享”把业务主链路稳定跑通。  
...
| WebRTC | WebRTC是该题关键技术点 | 面试要说明WebRTC的边界和取舍 |
| SFU | SFU是该题关键技术点 | 面试要说明SFU的边界和取舍 |
| 屏幕共享 | 屏幕共享是该题关键技术点 | 面试要说明屏幕共享的边界和取舍 |
| 录制 | 录制是该题关键技术点 | 面试要说明录制的边界和取舍 |
| 低延迟 | 低延迟是该题关键技术点 | 面试要说明低延迟的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Video Conferencing 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
### 15.1 幂等入口与主事务
```java
public class VideoConferencingCommandService {
...
### 15.2 状态机与边界校验
```java
public class VideoConferencingStateMachine {
...
### 15.3 失败重试与死信
```java
public class VideoConferencingRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class VideoConferencingQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 88 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class VideoConferencingReconcileJob {
...
export function VideoConferencingHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/88/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Video Conferencing System - 媒体上传台</h3>
      {msg && <p>{msg}</p>}
...
export function VideoConferencingOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/88/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/88/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Video Conferencing System - 播放质量看板</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Video Conferencing System 如何保证 WebRTC协议 不退化。
2. 当依赖超时时，Design Video Conferencing System 如何通过降级保证主功能可用。
3. 当 SFU架构 出现冲突时，如何通过幂等和补偿收敛。
4. 当 NAT穿透 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 屏幕共享 上升时，值班如何按 Runbook 处置。
...
2. 本题核心实体与索引设计围绕“WebRTC协议、SFU架构”展开。  
3. 失败模式重心不同：本题更常见“NAT穿透”相关故障。  
4. 性能热点位置不同：本题重点在“WebRTC”链路。  
5. 运营治理不同：本题要重点覆盖“SFU”告警与修复。  

### 22.4 本题新增必补知识（>=5条）
1. WebRTC协议 的实现边界与替代方案  
2. SFU架构 的一致性与容量取舍  
3. NAT穿透 的故障演练与止血策略  
4. 屏幕共享 的指标阈值与诊断路径  
5. 会议录制 的落地代码与运维闭环  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 WebRTC协议，这是最显著差异。”  
2. “本题在 WebRTC 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 SFU 的落地与故障处置。”  
