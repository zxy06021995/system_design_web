# Q71：Design Task Scheduler（95分面试版）

## 1. 三句话题目本质
1. Design Task Scheduler 的核心是：围绕“定时任务、延迟队列、任务调度”把业务主链路稳定跑通。  
2. 难点在于高峰、故障和多系统协同时，正确性、延迟和成本同时受压。  
3. 面试高分要点：先讲主链路，再讲异常链路，再给出可落地的取舍与演练方案。  

## 2. 一个真实场景故事
调度系统题核心是准时触发和失败恢复，难点在DAG依赖、重试风暴和补跑。
...
| 定时任务 | 定时任务是该题关键技术点 | 面试要说明定时任务的边界和取舍 |
| 延迟队列 | 延迟队列是该题关键技术点 | 面试要说明延迟队列的边界和取舍 |
| 任务调度 | 任务调度是该题关键技术点 | 面试要说明任务调度的边界和取舍 |
| 分布式 | 分布式是该题关键技术点 | 面试要说明分布式的边界和取舍 |
| 重试 | 重试是该题关键技术点 | 面试要说明重试的边界和取舍 |

## 4. 需求澄清（功能/非功能/不做范围）
### 4.1 功能需求
- 支持 Task Scheduler 的核心业务操作链路。
- 支持状态查询、异常处理、审计和运营控制。
- 支持扩展能力与多场景策略配置。
...
- `POST /jobs/{id}/backfill`
- `GET /jobs/{id}/runs`

## 8. 数据模型（核心表/索引）
- `job_def(job_id,cron,retry_policy)`
- `job_run(run_id,job_id,state)`
- `dag_edge(parent,child)`
...
### 15.1 幂等入口与主事务
```java
public class TaskSchedulerCommandService {
...
### 15.2 状态机与边界校验
```java
public class TaskSchedulerStateMachine {
...
### 15.3 失败重试与死信
```java
public class TaskSchedulerRetryWorker {
...
### 15.4 质量门禁与告警触发
```java
public class TaskSchedulerQualityGuard {
    public GuardResult evaluate(MetricsSnapshot s) {
        ...
        alertService.fire("Q" + 71 + "_QUALITY_FAIL",
...
### 15.5 对账与修复任务
```java
public class TaskSchedulerReconcileJob {
...
export function TaskSchedulerHealthPanel() {
  ...
  useEffect(() => {
    let t: number;
    const load = async () => {
      const resp = await fetch("/api/v1/71/health");
      if (resp.ok) {
  ...
  return (
    <div>
      <h3>Design Task Scheduler - 任务编排台</h3>
      {msg && <p>{msg}</p>}
...
export function TaskSchedulerOpsPanel() {
  const [msg, setMsg] = useState("");

  const replay = async () => {
    const resp = await fetch("/api/v1/71/ops/replay", { method: "POST" });
    setMsg(resp.ok ? "回放已触发" : "回放失败");
  };

  const rollback = async () => {
    const resp = await fetch("/api/v1/71/ops/rollback", { method: "POST" });
    setMsg(resp.ok ? "回滚已触发" : "回滚失败");
  };

  return (
    <div>
      <h3>Design Task Scheduler - 运行记录与重试页</h3>
...
## 18. 丰富例子（>=10）
1. 高峰流量下，Design Task Scheduler 如何保证 时间轮算法 不退化。
2. 当依赖超时时，Design Task Scheduler 如何通过降级保证主功能可用。
3. 当 延迟队列 出现冲突时，如何通过幂等和补偿收敛。
4. 当 分布式协调 放大 3 倍时，如何分层限流与资源扩容。
5. 当单点组件故障时，如何在 RTO 目标内恢复。
6. 当策略误发布时，如何灰度回滚并避免影响扩大。
7. 当告警显示 任务重试 上升时，值班如何按 Runbook 处置。
...
### 22.3 关键差异（>=5条）
1. 本题业务目标更聚焦“分布式系统”场景，非母题通用主线。  
2. 本题核心实体与索引设计围绕“时间轮算法、延迟队列”展开。  
3. 失败模式重心不同：本题更常见“分布式协调”相关故障。  
4. 性能热点位置不同：本题重点在“定时任务”链路。  
5. 运营治理不同：本题要重点覆盖“延迟队列”告警与修复。  
...
### 22.5 面试差异话术（3条）
1. “母题给我通用框架，这题我重点补了 时间轮算法，这是最显著差异。”  
2. “本题在 定时任务 上的约束更强，所以架构和阈值策略不同。”  
3. “如果只按母题回答会偏泛，我会补充本题在 延迟队列 的落地与故障处置。”  
